...
def __parse_line(line):
    # line = tf.Print(line, [line], message = "line is:", first_n =5)
    fields = tf.decode_csv( line, FIELD_DEFAULTS )
    features = dict(zip(COLUMNS, fields))
    print( "feature is:{0}\r\n".format( features ) )
    labels = features.pop('label')
    # labels = tf.Print(labels, [labels], message = "labels", first_n = 5)
    return features, labels

def data_input_function(PATH, BatchSize):
    print("**I'm input function**\r\n")
...
def my_model_fun( features, labels, mode, params ):
    ...
    # net = tf.Print(net, [net], message ="Input net", first_n = 1, summarize = 5)
...
classifier.train(input_fn = lambda: data_input_function(TrainDataPath, 2), hooks=None, steps=1)
